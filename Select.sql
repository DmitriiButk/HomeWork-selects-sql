...
select name , count(Artist_id) as Artist_quantity
from Genre as G
...
select Al."name", Al."year", COUNT(Track_id) as Track_quantity
from Album as Al
...
select Al."name" , AVG(duration) as avg_duration
from Album as Al
join Track as Tr on Al.album_id  = Tr.album_id 
group by Al."name"  
...
select Ar."name" , Al."year" 
from artist as Ar
join albums_artist as Aa on Ar.artist_id  = Aa.artist_id 
join Album as Al on Aa.album_id  = Al.album_id  
where Al."year" != 2020
group by Ar."name", al."year"  
